...
from pmma.mirp.dicomImport import read_dicom_image_series, read_dicom_rt_struct, read_roi_names, get_all_dicom_headers
from pmma.mirp.imageClass import ImageClass
from pmma.mirp.imageMetaData import get_sitk_dicom_meta_tag
from pmma.mirp.itkImport import read_itk_image, read_itk_segmentations
...
# from pmma.mirp.pydicom_fix import read_dataset
...
def find_imaging_parameters(image_folder, modality, subject, plot_images, write_folder,
    ...
    from pmma.mirp.imagePlot import plot_image
    from pmma.mirp.imageMetaData import get_meta_data
    from pmma.mirp.imageProcess import estimate_image_noise
...
def import_image(folder, modality=None, name_contains=None):
    ...
    # Find npy files                                                                                                    #####
    npy_file_list = [file_name for file_name in file_list if file_name.lower().endswith(".npy")]
    ...
    # load 3d data in one                                                                                               #####
    elif len(npy_file_list) > 0:
        img_obj = np.load(os.path.join(folder, npy_file_list[0]))
        img_obj = ImageClass(voxel_grid=img_obj,
                     origin=(0,0,0),
                     spacing=(1,1,1),
...
from pmma.mirp.roiClass import RoiClass                                                                                      #####
def import_segmentations(folder, image_object, roi_names):
    ...
    # load 3d data in one                                                                                               #####
        ...
        roi_file = [file_name for file_name in npy_file_list if file_name.startswith(roi_names[0])]
        roi_data = np.load(os.path.join(folder, roi_file[0]))
        # binarize roi_obj
        # roi_data = (roi_data > 0)
        roi_mask_obj = ImageClass(voxel_grid=roi_data,
                                  origin=(0,0,0),
                                  spacing=(1,1,1),
                                  orientation=np.array([1., 0., 0., 0., 1., 0., 0., 0., 1.]),
                                  modality="SEG",
                                  spat_transform="base",
                                  no_image=False)
        roi_list = [RoiClass(name='protons', contour=None, roi_mask=roi_mask_obj)]
    return roi_list
